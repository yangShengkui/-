        ...
        _this.click(function(event){
        	...
        	if(config.disableds == disableds){
        		...
        		if(disableds == false){
        			...
        			if(nowyear1 == getFullYear && nowmonth1 == getMonth){
        				...
        				//获取当月的最后一天
        				var lastday = $(".day li[data]").text().substr(-2);
        				
        				$(".day li[data]").each(function(index){
                        	var nowyear2 = disableds[0];
                        	var nowmonth2 = disableds[1];
                			var nowday2 = disableds[2];
                        	var nowselected2 = $(".day li[data]").eq(index).text();
                        	//小于当天前且大于明天不能选
                        	if(nowselected2 < nowday2 || nowselected2 > (nowday2+1)){
                        		$(".day li[data]").eq(index).css("pointer-events","none");
                        	//如果是当月最后一天则下个月只能选1号	
                        	}else if(lastday == nowday2){
                        		$("ul .next:first").css("pointer-events","auto");
                        	}
                        });
